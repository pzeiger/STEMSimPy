...
from stemsimpy._project import _Project
import warnings


class FrozenPhononMultislice(_Project):
    """ 
    """
    def __init__(self, projfile, inputyaml='simulation.yaml'):
        """
        """
        print('instantiating FrozenPhononMultislice()')
        super().__init__(projfile, inputyaml)
#        self._assemble_data(filelist)
        if inputyaml:
            self._init_from_yaml(inputyaml=inputyaml)
        else:
            warnings.warn('No inputyaml specified')
